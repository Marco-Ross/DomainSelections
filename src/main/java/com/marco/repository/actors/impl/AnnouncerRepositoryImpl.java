...
@Repository("AnnouncerFirstRepoImpl") //DAO(Data Access Object) fulfills the roll/stereotype of a repository | //@Repository specifies that this class is inside the IoC container and can be injected(wired) to other classes from within springs application ctx
public class AnnouncerRepositoryImpl implements AnnouncerRepository {
    ...
    private AnnouncerRepositoryImpl(){
    ...
    public Announcer update(Announcer announcer) {
        for(int i = 0; i<announcers.size();i++){
            if(announcer.getEmployeeNumber() == announcers.get(i).getEmployeeNumber()){
                this.announcers.set(i, announcer);
                return announcer;
            }
        }
        return null;
